...
function buildIframeEmbed(src: string) {
  return `<iframe src="${src}" width="100%" height="600" style="border:none;" sandbox="allow-same-origin allow-scripts allow-popups allow-forms allow-modals" title="App"></iframe>`;
}
...
serve(async (req) => {
  ...
  try {
    ...
    log("info", "request_received", {
      ...
      has_app_id: typeof app_id === "string" && app_id.length > 0,
    ...
    if (session.role !== "owner") {
      return respond(403, { error: "Only owners can manage apps" });
    }
    ...
    const uploadImageIfProvided = async (): Promise<string | null> => {
      ...
      const { error: uploadError } = await supabase.storage.from("app-images").upload(fileName, bytes, {
    ...
    switch (action) {
      case "add": {
        ...
        if (insertError) {
          log("error", "app_insert_failed", { message: insertError.message });
          return respond(500, { error: "Failed to add app" });
        }

        log("info", "app_added", { app_id: app.id });
        return respond(200, { success: true, app });
      }

      case "update": {
        if (!app_id) return respond(400, { error: "app_id required" });
        ...
        if (existingError || !existingApp) {
          return respond(404, { error: "App not found" });
        }
        ...
        if (newImageUrl) {
          ...
          if (existingApp.image_url) {
            const oldFileName = getFileNameFromPublicUrl(existingApp.image_url);
            if (oldFileName) {
              await supabase.storage.from("app-images").remove([oldFileName]);
            }
          }
        }
        ...
        if (updateError) {
          log("error", "app_update_failed", { message: updateError.message, app_id });
          return respond(500, { error: "Failed to update app" });
        }

        return respond(200, { success: true, app: updated });
      }
      ...
      case "delete": {
        ...
        if (existingApp?.image_url) {
          const fileName = getFileNameFromPublicUrl(existingApp.image_url);
          if (fileName) {
            await supabase.storage.from("app-images").remove([fileName]);
          }
        }

        const { error: deleteError } = await supabase.from("apps").delete().eq("id", app_id);

        if (deleteError) {
          log("error", "app_delete_failed", { message: deleteError.message, app_id });
          return respond(500, { error: "Failed to delete app" });
        }
